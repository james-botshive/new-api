...
func TestChannelFailKey(t *testing.T) {
	t.Run("single key", func(t *testing.T) {
		assert.Equal(t, "42", channelFailKey(42, ""))
	})
	t.Run("multi key", func(t *testing.T) {
		assert.Equal(t, "42:sk-abc123", channelFailKey(42, "sk-abc123"))
	})
}
...
func TestHandleChannelFailure_Disabled(t *testing.T) {
	...
	HandleChannelFailure(*chErr, "test error")

	state := GetChannelFailureState(1, "")
...
func TestHandleChannelFailure_ThresholdZero(t *testing.T) {
	...
	state := GetChannelFailureState(2, "")
...
func TestHandleChannelFailure_CountsCorrectly(t *testing.T) {
	...
	channelId := 100
	defer ResetChannelFailure(channelId, "")

	chErr := types.NewChannelError(channelId, 1, "counting-channel", false, "", true)

	// First 3 failures: threshold not reached
	for i := 0; i < 3; i++ {
		HandleChannelFailure(*chErr, "error "+strconv.Itoa(i+1))
	}

	state := GetChannelFailureState(channelId, "")
...
func TestHandleChannelFailure_ThresholdCrossing(t *testing.T) {
	...
	// First 2 failures
	HandleChannelFailure(*chErr, "error 1")
	HandleChannelFailure(*chErr, "error 2")
	...
	HandleChannelFailure(*chErr, "error 3")
...
func TestHandleChannelFailure_AlertDedup(t *testing.T) {
	...
	HandleChannelFailure(*chErr, "error 3")
	HandleChannelFailure(*chErr, "error 4")
...
func TestResetChannelFailure(t *testing.T) {
	...
	HandleChannelFailure(*chErr, "new error 1")
	HandleChannelFailure(*chErr, "new error 2")
...
func TestHandleChannelFailure_MultiKeyIsolation(t *testing.T) {
	...
	channelId := 600
	defer ResetChannelFailure(channelId, "key-a")
	defer ResetChannelFailure(channelId, "key-b")
	...
	HandleChannelFailure(*chErrA, "error a1")
	HandleChannelFailure(*chErrA, "error a2")

	stateA := GetChannelFailureState(channelId, "key-a")
	require.NotNil(t, stateA)
	assert.True(t, stateA.alerted)

	// Key B should be independent
	stateB := GetChannelFailureState(channelId, "key-b")
	assert.Nil(t, stateB, "key-b should be independent")

	// Key B starts failing
	HandleChannelFailure(*chErrB, "error b1")
	stateB = GetChannelFailureState(channelId, "key-b")
	...
	for i := 0; i < goroutines; i++ {
		go func() {
			...
			for j := 0; j < incrementsPerGoroutine; j++ {
				HandleChannelFailure(*chErr, "concurrent error")
			}
		}()
	}
	wg.Wait()
...
func TestGetChannelFailureState_Unknown(t *testing.T) {
	state := GetChannelFailureState(99999, "")
